...
## 커스텀(범용) API 등록 — HIRA/법제처 외에 어떤 API든

"API 키 설정" 화면 아래쪽 **"커스텀 API 관리"** 섹션에서 HIRA/법제처처럼
코드로 미리 만들어두지 않은 임의의 API도 등록해 쓸 수 있다. 등록할 때
입력하는 항목:
...
| 인증 키 값 | 실제 발급받은 키 |
| 검색어 파라미터 이름 | 사용자의 검색어를 실어 보낼 파라미터 이름(선택, 비워두면 검색어 없이 고정 파라미터로만 호출) |
| 고정 파라미터 | 항상 붙일 값. `dataType=JSON&numOfRows=20` 형식 |

등록 즉시(앱 재시작 없이) 다음 질문부터 쓸 수 있다. 지원 범위는 "검색어
하나 + 고정 파라미터"로 호출하고, 응답 JSON에서 배열처럼 보이는 부분을
찾아 표로 보여주는 수준의 범용 Connector(`connectors/generic/customApiConnector.ts`)
라서, HIRA/법제처 Connector처럼 필드별로 깔끔하게 정리되진 않는다.
...
- **GGUF 모델을 올려서 쓰는 경우**: 등록된 모든 API(HIRA/법제처 + 커스텀)의
  이름과 설명이 모델의 시스템 프롬프트에 그대로 나열되므로
...
- **모델을 안 올리고 규칙 기반 폴백으로 쓰는 경우**
  ...
  3. 그 외엔 커스텀 API의 이름+설명과 질문의 단어가 2개 이상 겹치면 그
     API로 보낸다. 뚜렷하게 겹치는 게 없으면 잘못 추측하는 대신 기존
     기본값(병원 검색)으로 처리한다.

즉 "질문에 서비스 이름을 넣으면 확실하게 그 API로, 안 넣어도 설명이
구체적이면 어느 정도는 알아서" 동작한다. 애매한 질문을 항상 정확히
분류해주진 않으므로, 커스텀 API를 자주 쓸 계획이면 설명을 구체적으로
적어두거나 질문에 서비스 이름을 넣어주는 편이 안전하다.
