...
describe("ConversationManager", () => {
  it("대화를 생성하고 메시지를 순서대로 저장한다", async () => {
    const manager = new ConversationManager(await openInMemoryDatabase());
  ...
  it("현재 대화를 삭제하면 메시지도 함께 사라진다 (CASCADE)", async () => {
    const manager = new ConversationManager(await openInMemoryDatabase());
