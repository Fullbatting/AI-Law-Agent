...
describe("CacheManager", () => {
  it("캐시가 없으면 undefined를 반환한다", () => {
    const cache = new CacheManager(openInMemoryDatabase());
    expect(cache.get(dsl)).toBeUndefined();
  });

  it("저장한 결과를 동일한 DSL로 다시 조회할 수 있다", () => {
    const cache = new CacheManager(openInMemoryDatabase());
  ...
  it("만료된 캐시는 조회 시 삭제되고 undefined를 반환한다", () => {
    const cache = new CacheManager(openInMemoryDatabase(), { hira: -1 });
    cache.set(dsl, makeResult());
    expect(cache.get(dsl)).toBeUndefined();
  });

  it("clearAll 이후에는 모든 캐시가 사라진다", () => {
    const cache = new CacheManager(openInMemoryDatabase());
    cache.set(dsl, makeResult());
    cache.clearAll();
    expect(cache.get(dsl)).toBeUndefined();
